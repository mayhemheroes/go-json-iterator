#!/usr/bin/env bash
#
# go-json-iterator/mayhem/test.sh — RUN json-iterator/go's OWN Go test suite (`go test ./...`)
# and emit a CTRF summary. exit 0 iff no test failed.
#
# PATCH-grade oracle: jsoniter's suite is a REAL known-answer / golden-output suite — the
# api_tests, type_tests, value_tests, misc_tests, skip_tests, any_tests and extension_tests
# packages assert that jsoniter's decode/encode output equals the encoding/json reference for
# hundreds of typed values (require.Equal against the stdlib result). They assert BEHAVIOUR, not
...
# and thus immune to the LD_PRELOAD sabotage mechanism), this script also executes /mayhem/fuzz_json
# (dynamically linked, ASan+libFuzzer) against a known corpus entry and asserts specific libFuzzer
# output strings ("Executed ... in"). A no-op / exit(0) PATCH to json-iterator's parser leaves
# fuzz_json intact (it IS the compiled Go parser), so it still emits the expected output. When the
# SABOTAGE MECHANISM (LD_PRELOAD _exit(0)) neuters fuzz_json itself, fuzz_json exits silently and
...
# ── Behavioral probe via the dynamically-linked fuzz_json binary (anti-reward-hacking, §6.3) ──
# Go test binaries are statically linked, so the LD_PRELOAD sabotage mechanism cannot neuter them.
# /mayhem/fuzz_json IS dynamically linked (built with clang+ASan). Run it single-shot against a
# known corpus entry and assert that libFuzzer emits "Executed" — proving it actually processed
# the input. The sabotage LD_PRELOAD neuters fuzz_json (not in /usr/bin etc.), causing it to
# exit silently → the grep fails → FAILED increments → the oracle is NOT reward-hackable.
PROBE_INPUT="$SRC/mayhem/fuzz_json/testsuite/mixed.json"
if [ -x /mayhem/fuzz_json ] && [ -f "$PROBE_INPUT" ]; then
  echo "=== behavioral probe: fuzz_json single-shot on known corpus ==="
  PROBE_OUT=$(/mayhem/fuzz_json "$PROBE_INPUT" 2>&1 || true)
  if echo "$PROBE_OUT" | grep -q "Executed"; then
    echo "PROBE PASS: fuzz_json executed the corpus input (parser active)"
    PASSED=$(( PASSED + 1 ))
  else
    echo "PROBE FAIL: fuzz_json produced no 'Executed' output (parser inactive or sabotaged)"
