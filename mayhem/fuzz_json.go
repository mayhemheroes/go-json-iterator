...
//
// This is the OSS-Fuzz harness for json-iterator/go (OSS-Fuzz project
// "go-json-iterator", target func Fuzz). It is a LEGACY go-fuzz harness:
//   func Fuzz(data []byte) int   -> built with go114-fuzz-build (`go-fuzz`).
//
// It lives in package jsoniter and is gated behind the `gofuzz` build tag so it
// only compiles for the fuzz build (matching OSS-Fuzz's `compile_go_fuzzer ...
// Fuzz fuzz_json gofuzz`) and never pollutes the normal `go test ./...` suite.
//
// The harness is a differential oracle: it round-trips arbitrary input through
// jsoniter (ConfigCompatibleWithStandardLibrary) and compares against the Go
// standard library encoding/json. A divergence (decode result, marshal/remarshal
// fidelity) panics — a real correctness oracle, not just a crash sink.
